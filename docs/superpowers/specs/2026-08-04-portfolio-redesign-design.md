...
現在の my-apps-portal は自分向けの保守ダッシュボードである。21 アプリを 9 列のテーブルに並べ、npm registry・OSV・GitHub API から latest / outdated / vulnerable を判定して「どのアプリを直すべきか」を可視化している。
...
│  [職務経歴書 ↗]  [GitHub ↗]
├─ サマリ（肯定形の集計のみ）──────────────
│  21作品 · テスト3,081 · 脆弱性0 · Lighthouse平均98.5
├─ 代表作 ─────────────────────────────────
...
`projects.ts` の実データから実行時に計算する。手書きの数値は必ず陳腐化する（現に `layout.tsx` の description は「全 11 プロジェクト」のまま実際は 21 件）。
...
`projects.ts` では 21 件すべてが `githubVisibility: "public"` になっているが、実際の GitHub 上の公開状態と一致しているか未検証である。ポートフォリオとして公開する以上、private リポジトリを public と表示するのは避けたい。実装時に `gh repo view` で全 21 件の実際の可視性を照合し、齟齬があれば修正する。
...
7. 全 21 件の `githubVisibility` が実際の GitHub の状態と一致している
